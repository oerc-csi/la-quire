...
module.exports = function(eleventyConfig, { page }) {
  ...
  return function (pageObjects = []) {
    ...
    const tableRow = (object, property) => {
      const propertyName = config[property] || property // Use configured field names
      if (!object || !propertyName || !object[propertyName]) return ''

      return html`
        <tr>
          <td>${titleCase(propertyName)}</td>
          <td>${markdownify(object[propertyName].toString())}</td>
        </tr>
      `
    }
