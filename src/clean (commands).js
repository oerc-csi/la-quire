 ...
 * to allow different behaviors for Quire editors and developers.
 *
 * @class      CleanCommand
 * @extends    {Command}
 */
export default class CleanCommand extends Command {
  static definition = {
    ...
    options: [
      ...
      ['-f, --force [keepLa]', 'run clean without removing linked-art.json or figures.json'],
    ],
  };
  ...
  async action(options, command) {
    ...
    // Determine whether to keep linked-art.json based on the value of keepLa
    const keepLa = options.force === 'keepLa';

    // Pass the keepLa option to the clean function
    const deletedPaths = await clean(projectRoot, paths, options, keepLa);
